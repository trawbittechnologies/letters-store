  ...
  return (
    <div className="bg-[var(--bg)] min-h-screen pt-0 pb-24 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto space-y-10 px-4 sm:px-6 lg:px-12 pt-8">
        ...
        {/* ========================================================================= */}
        {/* 3. QUICK DISCOUNT BENTO CATEGORY CARDS */}
        {/* ========================================================================= */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          {[
            ...
            <button
              ...
              className={`group text-left card-minimal p-3 sm:p-3.5 transition-all duration-300 cursor-pointer overflow-hidden flex flex-col justify-between border ${
                selectedFilter === card.id
                  ? 'border-[var(--text)] ring-2 ring-[var(--chandanam)]/40 bg-[var(--card)] shadow-md'
                  : 'bg-[var(--card)] hover:border-[var(--chandanam)]/60 hover:shadow-md'
              }`}
            >
              <div className="relative aspect-[16/10] rounded-xl overflow-hidden bg-[var(--bg-subtle)] mb-2.5">
                <img
                  src={card.img}
                  alt={card.title}
                  className="w-full h-full object-cover group-hover:scale-108 transition-transform duration-600 ease-out"
                />
                <span className="absolute top-2 left-2 text-[9px] font-extrabold bg-[#721C28] text-white px-2 py-0.5 rounded shadow-sm">
                  {card.badge}
                </span>
              </div>
              <div>
                <h3 className="font-heading text-[13.5px] font-bold text-[var(--text)] group-hover:text-[var(--maroon)] transition-colors leading-tight">
                  {card.title}
                </h3>
                <p className="text-[var(--text-muted)] text-[10.5px] mt-0.5 line-clamp-1">
                  {card.sub}
                </p>
              </div>
            </button>
